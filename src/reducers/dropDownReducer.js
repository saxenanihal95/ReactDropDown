...
function updatedItemList(type, operationType, itemList) {
  ...
  const { adultCount, childCount, babyCount, totalCount } = getUserCounts(
    itemList
  );
...
function updateDisplayText(itemList) {
  ...
  const childText =
    childCount > 0
      ...
      : "";
  const babyText =
    babyCount > 0 ? (babyCount === 1 ? "1 Baby" : `${babyCount} Babies`) : "";

  const displayText = [adultText, childText, babyText]
    .filter(t => t)
    .join(", ");
  return displayText;
}
